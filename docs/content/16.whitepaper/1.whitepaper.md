---
title: aster Technical Whitepaper
description: aster - 企业级AI Agent开发框架技术白皮书
navigation:
  icon: i-lucide-file-text
---

# aster: 企业级AI Agent开发框架
...
随着大语言模型（LLM）技术的快速发展，AI Agent已成为构建智能应用的核心范式。然而，现有的Agent框架大多面临生产就绪性不足、性能瓶颈、安全风险等挑战。本文介绍aster，一个基于Go语言的企业级AI Agent开发框架，专注于解决生产环境中的实际问题。
...
4. **生产级性能**：基于stream.Reader流式接口，内存占用降低80%+，支持100+并发Agent，Middleware Stack性能达到36.21 ns/op。
...
aster已在多个生产环境中部署，支撑了客户服务、文档处理、自动化运维等多种企业级应用场景。
...
- **流式API**：基于stream.Reader，内存占用降低80%+
- **零拷贝优化**：减少不必要的数据复制
...
传统Agent框架采用单通道的消息传递，难以区分不同类型的事件。aster创新性地提出**三通道设计**：
...
借鉴Web框架（如Koa.js）的中间件设计，aster实现了**洋葱模型**：
...
aster完整实现了Google "Context Engineering: Sessions, Memory" 白皮书的所有核心功能，评分**100/100**：
...
- **MultiAgent**：多Agent协作（Cosmos）

**核心流程**：

```go
// 简化的Agent运行流程
func (a *BaseAgent) Run(ctx context.Context, input string) *stream.Reader[Event] {
    reader, writer := stream.Pipe[Event](10)
    go func() {
        defer writer.Close()
...
aster使用`stream.Reader`实现高效的流式处理：
...
// stream.Reader方式：惰性生成，零内存开销
func (a *Agent) Run(ctx context.Context, input string) *stream.Reader[Event] {
    reader, writer := stream.Pipe[Event](10)
    go func() {
        defer writer.Close()
        for {
...
aster借鉴了Web框架（如Koa.js、Express.js）的中间件设计理念，实现了**洋葱模型（Onion Model）**。这种设计允许在Agent执行的Before和After阶段插入自定义逻辑。
...
func (m *SubAgentMiddleware) Before(ctx context.Context, req *AgentRequest) (*AgentRequest, error) {
    ...
    for _, task := range tasks {
        if task.ShouldDelegate() {
            // 创建专用子Agent
            subAgent, _ := m.cosmos.Get(task.Type)
...
aster实现了完整的三层记忆系统，并完整支持Google Context Engineering白皮书的所有核心功能。
...
aster完整实现了Google Context Engineering白皮书的所有核心功能，提供生产级的上下文管理能力。
...
安全和隐私是企业级AI Agent的基石。aster提供多层防护机制。

### 7.1 Sandbox Systems（沙箱系统）

aster支持三种Sandbox实现，隔离代码执行环境：
...
现代AI应用往往需要多个Agent协作完成复杂任务。aster提供了完整的多Agent管理和协作框架，包括 Cosmos（生命周期管理）、Scheduler（任务调度）和 Workflow（工作流编排）三大核心组件。

### 8.1 Cosmos：Agent 生命周期管理
...
Cosmos 提供**统一的Agent生命周期管理**，类似于数据库连接池，但针对Agent实例优化。

#### 8.1.2 核心能力

**1. 容量控制**

Cosmos 设置了默认最大容量（50个Agent），防止资源耗尽：
...
Scheduler 提供智能的任务调度能力，支持：

- **步骤触发**：每N步执行一次任务
- **时间触发**：定时执行任务
- **事件监听**：监听Agent步骤完成事件

详细文档请参考 [Scheduler 文档](/multi-agent/scheduler)
...
aster提供三种WorkflowAgent，支持不同的任务分发模式：
...
aster的多Agent系统体现了以下设计理念：

**1. 松耦合架构**

Agent 通过 Cosmos 统一管理，而非直接耦合：
...
- Scheduler 异步调度（channel）
- Cosmos 无锁读取（RWMutex）

## 9. Provider & Tool System

LLM Provider和Tool System是Agent的两大基础设施。aster通过**Protocol（协议）+ Factory（工厂）**模式实现了高度解耦的设计，支持10+个LLM厂商和9个内置工具，同时通过MCP协议支持无限扩展。
...
aster采用**Provider Protocol**设计，将LLM抽象为统一接口：
...
aster通过`MultiProviderFactory`支持**10+个主流LLM厂商**：
...
aster提供**9个生产级内置工具**，覆盖文件系统、Shell、网络三大类：
...
LLM根据Schema生成正确的参数格式，aster根据Schema验证输入。
...
aster通过MCP适配层支持**任意MCP Server**。

#### 9.3.2 命名空间隔离

当Agent连接多个MCP Server时，可能存在工具名称冲突。aster通过**命名空间**解决：
...
aster的Provider & Tool System体现了以下优势：
...
Skills 系统是 aster 的**动态知识库注入机制**，允许在不修改系统提示词或重新训练模型的情况下，为 Agent 添加领域专业知识、工作流程规范和最佳实践。
...
| `pkg/cosmos/`     | 200      | 1      | Cosmos 生命周期管理 |
...
- `StreamCollect(reader)`：收集所有事件到切片
- `StreamFirst(reader)`：获取第一个事件
- `StreamLast(reader)`：获取最后一个事件
- `StreamFilter(reader, predicate)`：过滤事件
- `StreamMap(reader, transform)`：转换事件

**性能数据**：

| 场景      | 传统方式 | stream.Reader | 内存降低 |
| --------- | -------- | ------------- | -------- |
| 100事件   | 4KB      | 40B           | 99%      |
...
| 组件   | 读操作    | 写操作         | 保护对象   |
| ------ | --------- | -------------- | ---------- |
| Cosmos | Get, List | Create, Delete | agents map |
...
aster包含**21个Benchmark测试**，覆盖关键性能路径：
...
**Context相关（5个）**：8. `BenchmarkSimpleTokenCounter_Count` 9. `BenchmarkSimpleTokenCounter_EstimateMessages` 10. `BenchmarkSlidingWindowStrategy_Compress` 11. `BenchmarkPriorityBasedStrategy_Compress` 12. `BenchmarkTokenBasedStrategy_Compress`

**Provider相关（2个）**：13. `BenchmarkProviderCreation` 14. `BenchmarkMessageConversion`

**其他核心路径（7个）**：15. `BenchmarkExtractMessageContent` 16. `BenchmarkMiddlewareStack` 17. `BenchmarkBackendWrite` 18. `BenchmarkPathValidation` 19. `BenchmarkPathValidation_Disabled` 20. `BenchmarkFormatContentWithLineNumbers` 21. `BenchmarkSanitizeToolCallID`
...
aster提供**31个生产级示例**，覆盖从入门到高级的完整路径：
...
**多Agent协作（4个）**：4. **examples/cosmos/**：Cosmos 生命周期管理 5. **examples/subagent/**：子Agent委托6. **examples/workflow-agents/**：三种工作流（Parallel/Sequential/Loop）7. **examples/router/**：模型路由选择

**记忆系统（7个）**：8. **examples/memory/**：Text Memory基础9. **examples/memory-agent/**：Agent Memory集成10. **examples/memory-working/**：Working Memory使用11. **examples/memory-advanced/**：高级记忆（Provenance/Consolidation）12. **examples/memory-semantic/**：Semantic Memory（向量检索）13. **examples/memory-semantic-session/**：Session + Semantic Memory 14. **examples/agent-working-memory/**：Agent + Working Memory完整示例

**数据持久化（3个）**：15. **examples/session/**：内存会话（StateBackend）16. **examples/session-postgres/**：PostgreSQL持久化17. **examples/session-mysql/**：MySQL持久化

**工具扩展（3个）**：18. **examples/mcp/**：MCP协议集成19. **examples/long-running-tools/**：长时运行工具管理20. **examples/skills/**：Skills注入（提示词工程）

**网络功能（2个）**：21. **examples/server-http/**：HTTP Server包装Agent 22. **examples/cloud-sandbox/**：云沙箱（阿里云/火山引擎）

**评估与监控（4个）**：23. **examples/evals/**：评估框架24. **examples/evals-session/**：Session评估25. **examples/telemetry/**：OpenTelemetry集成26. **examples/logging/**：结构化日志

**其他（5个）**：27. **examples/scheduler/**：任务调度28. **examples/workflow-semantic/**：语义工作流
29-31. 其他高级场景
...
| 特性             | aster (Go)                                                  | LangChain                                 | AutoGPT         | Semantic Kernel       |
| ---------------- | ----------------------------------------------------------- | ----------------------------------------- | --------------- | --------------------- |
| **语言**         | Go                                                          | Python                                    | Python          | C#/Python             |
| **架构**         | 事件驱动 + 中间件                                           | Chain链式                                 | 自主循环        | Kernel + Plugin       |
| **流式处理**     | ✅ stream.Reader (O(1)内存)                                 | ⚠️ 部分支持 (O(n)内存)                    | ❌              | ⚠️ 部分支持           |
...
- Stream返回stream.Reader
- 内存占用O(1)
- 支持背压控制（客户端随时停止）

**性能数据**：
| 事件数 | LangChain内存 | aster内存 | 降低比例 |
|--------|--------------|-------------|----------|
| 1,000 | 40KB | 40B | 99.9% |
| 10,000 | 400KB | 40B | 99.99% |
...
#### 14.1.3 stream.Reader是游戏规则改变者

**决策**：基于stream.Reader重构流式接口
...
| stream.Reader | O(n)内存  | O(1)内存     | 80%+降低 |
...
aster是一个**生产就绪、高性能的AI Agent开发框架**，基于Go语言构建，专注于解决企业级应用的实际需求。
...
8. Go Team. "Go 1.23 Release Notes." August 2024. https://go.dev/doc/go1.23
...
**最后更新**：2025年11月
**许可证**：MIT License
**项目地址**：https://github.com/astercloud/aster
**文档站点**：https://aster.dev
