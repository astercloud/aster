...
aster 提供了分层的 Memory 管理系统，帮助 Agent 在会话内外保持状态和记忆：
...
```go
import "github.com/astercloud/aster/pkg/memory"
...
err = semantic.Index(ctx,
    "doc-123",  // 文档 ID
    "aster provides two memory systems...",  // 文本
...
1. 检查 `aster.yaml` 中 `memory.working_memory.enabled` 是否为 `true`
