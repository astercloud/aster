<template>
  <div class="aster-chat flex flex-col h-full bg-background dark:bg-background-dark">
    ...
    <!-- Messages -->
    <MessageList ref="messageListRef" :messages="messages" :is-typing="isTyping" :config="config" class="flex-1" />

    <!-- Quick Replies -->
    <QuickReplies v-if="quickReplies.length > 0" :replies="quickReplies" @select="handleQuickReply" class="flex-shrink-0" />
