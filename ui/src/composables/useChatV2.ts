...
export function useChat(config: ChatConfig) {
  ...
  onMounted(async () => {
    ...
    if (config.welcomeMessage && chatStore.messages.length === 0) {
      const welcomeText = typeof config.welcomeMessage === "string" ? config.welcomeMessage : config.welcomeMessage.type === "text" ? config.welcomeMessage.content.text : "👋 你好，我是 Aster Copilot。";
